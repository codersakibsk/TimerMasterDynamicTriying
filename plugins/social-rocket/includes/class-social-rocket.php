...
class Social_Rocket {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'social-rocket', plugin_dir_url( dirname( __FILE__ ) ) .'js/script.js', array( 'jquery' ), SOCIAL_ROCKET_VERSION, true );
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( 'social_rocket', plugin_dir_url( dirname( __FILE__ ) ) . 'css/style.css', array(), SOCIAL_ROCKET_VERSION, 'all' );
		
		if ( ! $this->settings['disable_fontawesome'] ) {
			wp_enqueue_style( 'fontawesome_core', plugin_dir_url( dirname( __FILE__ ) ) . 'css/fontawesome.min.css', array(), SOCIAL_ROCKET_VERSION, 'all' );
			wp_enqueue_style( 'fontawesome_brands', plugin_dir_url( dirname( __FILE__ ) ) . 'css/fa-brands.min.css', array( 'fontawesome_core' ), SOCIAL_ROCKET_VERSION, 'all' );
			wp_enqueue_style( 'fontawesome_regular', plugin_dir_url( dirname( __FILE__ ) ) . 'css/fa-regular.min.css', array( 'fontawesome_core' ), SOCIAL_ROCKET_VERSION, 'all' );
			wp_enqueue_style( 'fontawesome_solid', plugin_dir_url( dirname( __FILE__ ) ) . 'css/fa-solid.min.css', array( 'fontawesome_core' ), SOCIAL_ROCKET_VERSION, 'all' );
		}
	...
	public function get_floating_buttons_html( $args = array() ) {
		...
		foreach ( $networks as $network_key => $network_name ) {
			...
			$cur .= '<a class="social-rocket-floating-button-anchor'
				...
				. ' target="_blank"'
				. ' aria-label="' . $this->get_cta( $network_key, $settings ) . '">';
			
			// icon
			$cur .= '<span class="social-rocket-floating-button-icon">'
				. '<i class="' . $this->get_icon_class( $network_key, $settings ) . '">' . $this->get_icon_svg( $network_key, $settings ) . '</i>'
				. '</span>';
			
			// call to action
			if ( $show_cta ) {
				$cur .= '<span class="social-rocket-floating-button-cta">' . $this->get_cta( $network_key, $settings ) . '</span>';
			}
	...
	public function get_inline_buttons_html( $args = array() ) {
		...
		foreach ( $networks as $network_key => $network_name ) {
			...
			// icon
			$cur .= '<i class="'. $this->get_icon_class( $network_key, $settings ) . ' social-rocket-button-icon">' .
				$this->get_icon_svg( $network_key, $settings ) .
				'</i>';
			
			// call to action
			if ( $show_cta ) {
				$cur .= '<span class="social-rocket-button-cta">' . $this->get_cta( $network_key, $settings ) . '</span>';
			}
	...
	public function maybe_add_og_tags() {
		...
		if ( ! $this->_isset( $this->settings['disable_og_tags'] ) ) {
			...
			$og_image_height = apply_filters( 'social_rocket_og_image_width', ( is_array( $og_image_full ) ? $og_image_full[2] : false ) );
	...
	public function register_blocks() {
		...
		wp_register_script(
			...
			plugin_dir_url( dirname( __FILE__ ) ) . 'js/blocks.js',
			array( 'wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor' ),
			filemtime( SOCIAL_ROCKET_PATH . 'js/blocks.js' )
		);
	...
	public static function update_count_data( $id = 0, $type = 'post', $data ) {
